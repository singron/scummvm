...
bool OpenGLSdlGraphicsManager::loadGFXMode() {
	...
	if (!_screenResized) {
		...
		_videoMode.hardwareWidth = _videoMode.screenWidth * scaleFactor;
		_videoMode.hardwareHeight = _videoMode.screenHeight * scaleFactor;
		...
		if (_videoMode.aspectRatioCorrection) {
		 	if (_videoMode.screenWidth == 320 && _videoMode.screenHeight == 200)
				_videoMode.hardwareHeight = 240 * scaleFactor;
			else if (_videoMode.screenWidth == 640 && _videoMode.screenHeight == 400)
				_videoMode.hardwareHeight = 480 * scaleFactor;
		}
	}
